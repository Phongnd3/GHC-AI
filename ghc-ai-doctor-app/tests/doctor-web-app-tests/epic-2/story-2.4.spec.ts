...
test.describe('Story 2.4 - Automatic Session Timeout After 30 Minutes of Inactivity', () => {
  ...
  test('Session expires after 30 minutes of inactivity', async ({ page }) => {
    ...
    await loginPage.simulateSessionTimestamp(31);
    await page.reload();
    await expect(loginPage.getSessionExpiredText()).toContain('Session expired. Please log in again.');
    await expect(loginPage.usernameInput).toBeVisible();
  });

  test('User interaction resets inactivity timer', async ({ page }) => {
    await loginPage.login(VALID_USERNAME, VALID_PASSWORD);
    await dashboardPage.waitForDashboard();
    await page.click('body');
    await loginPage.simulateSessionTimestamp(29);
    await page.reload();
    await dashboardPage.waitForDashboard();
    await expect(page).toHaveURL(/.*dashboard/);
  });

  test('Session does not expire prematurely', async ({ page }) => {
    await loginPage.login(VALID_USERNAME, VALID_PASSWORD);
    await dashboardPage.waitForDashboard();
    await loginPage.simulateSessionTimestamp(29);
    await page.reload();
